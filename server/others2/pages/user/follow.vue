<template>
    <div class="flex-container">
        <div class="top">
            <van-nav-bar left-arrow @click-left="onClickLeft" title="关注" style="z-index: 1000; width: 100%" :placeholder="true"></van-nav-bar>
        </div>
                ...
                <div class="list" v-if="infoList.length > 0">
                    <van-pull-refresh v-model="b_loadingInfo" @refresh="onRefresh">
                        <van-list v-model:loading="b_loadingInfo2" :finished="loadNext_finished" finished-text="没有更多了" @load="onLoadNextPage">
                            <li class="item" v-for="(item, index) in infoList" :key="index" @click="onClickFollow(item.uid)">
                                <div>
                                    <div class="avatar">
...
export default {
    ...
    mounted() {
        console.log("follow mounted");
        this.$parent.showTabbar = false;
        this.loadUserFollows();
    },
    activated() {
        console.log("follow activated");
