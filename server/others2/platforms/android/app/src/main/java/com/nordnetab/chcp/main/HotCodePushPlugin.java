...
public class HotCodePushPlugin extends CordovaPlugin {
    ...
    private void fetchUpdate(CallbackContext jsCallback, FetchUpdateOptions fetchOptions) {
        ...
        String contentURL = chcpXmlConfig.getContentUrl();
        if(configURL == null || configURL == "") configURL = URLUtility.construct(contentURL, PluginFilesStructure.CONFIG_FILE_NAME);
    ...
    private void redirectToLocalStorageIndexPage() {
        ...
        if(strippedIndexPage.startsWith("http")) {
            webView.getEngine().setWWWDir(wwwFolder);
            webView.loadUrlIntoView(strippedIndexPage, false);
            Log.d("CHCP", "Loading page: " + strippedIndexPage);
        }else
        {
            if (!preferences.getBoolean("AndroidInsecureFileModeEnabled", false)) {
                Log.d("CHCP", "External AndroidInsecureFileModeEnabled is false, webview  not support file://.");
                return;
            }
            ...
            webView.loadUrlIntoView(external, false);
    ...
    @Subscribe
    public void onEvent(final DownloadErrorEvent event) {
        final PluginResult result = PluginResultHelper.pluginResultFromEvent(event);
        Log.d("CHCP", "DownloadErrorEvent  "+result.getMessage());
        sendMessageToDefaultCallback(result);
    }
