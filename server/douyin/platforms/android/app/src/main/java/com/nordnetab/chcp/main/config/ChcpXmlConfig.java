...
public class ChcpXmlConfig {
    ...
    /**
     * Getter for url to base config, that stored on server.
     * @return url to server file base config
     */
     ...
     * Setter for config url on server.
     *
     * @param contentUrl url to application config
     */
    ...
    public void mergeOptionsFromJs(JSONObject jsOptions) throws JSONException {
        ...
        if (jsOptions.has(XmlTags.CONTENT_URL_TAG)) {
            String contentUrl = jsOptions.getString(XmlTags.CONTENT_URL_TAG);
