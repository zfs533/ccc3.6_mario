...
export default {
    ...
    methods: {
        ...
        async loadData() {
            ...
            let res = await this.$http(getGDaySum, { page: this.page, count: this.count, ...this.getQuery() });
            if (res.code === 200) {
                if (isArray(res.msg.totalData) && res.msg.totalData.length > 0) {
        ...
        async exportGDaySum() {
            let res = await this.$http(exportGDaySum, { page: this.page, count: this.count, ...this.getQuery() });
            if (res.code === 200) {
                // this.pageData = res.msg.pageData;
                // this.totalCount = res.msg.totalCount;
