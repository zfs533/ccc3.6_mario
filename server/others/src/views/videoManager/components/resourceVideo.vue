<template>
    <el-card>
        ...
        <el-table :data="pageData" :border="true" min-width="100%" ref="table" @header-click="handleHeaderClick" @selection-change="handleSelectionChange" max-height="800px">
            ...
            <el-table-column v-if="showColumns.includes('pid')" fixed prop="pid" label="项目"  align="center" :formatter="$pidFormat" min-width="170"></el-table-column>
            <el-table-column v-if="showColumns.includes('_id')" fixed prop="_id" label="视频id" align="center" min-width="220"></el-table-column>
            ...
            <el-table-column v-if="showColumns.includes('index')" prop="index" label="排序索引" align="center" ></el-table-column>
            <el-table-column fixed="right" label="操作" width="150" align="center">
                <template slot-scope="scope">
                    <el-button type="text" style="margin:0 10px 0 0" @click="play(scope.row.playURL)">播放</el-button>
            ...
            <el-form label-width="80px" :inline="true">
                ...
                <el-form-item label="视频标签">
                    ...
                    <div :border="true" style=" display: inline-block !important;margin: 5px 0px;border:1px solid #ddd;width:100%">
                        <input style="margin:0 10px" ref="tagInput" v-if="!readOnly" class="tag-input" placeholder="请输入" @input="inputTag" :value="newTag" @keydown.delete.stop="removeLastTag" />
        ...
        <el-dialog title="批量编辑" :visible.sync="blukDialogVisible" width="600px" @close="closeBlukView">
            <el-form label-width="80px" :inline="true">
                ...
                <!-- <el-form-item  label="添加平台">
                    <el-select multiple v-model="blukEditObj.pid" placeholder="请选择项目">
                        <el-option v-for="item in pidList" :key="item.pid" :label="item.name" :value="item.pid"></el-option>
                    </el-select>
        ...
        <el-dialog title="克隆内容" :visible.sync="cloneDialogVisible" width="600px">
            <h5>克隆内容不存在分类和标签关联</h5>
            <el-form>
                <el-form-item  label="项目">
                    <el-select v-model="formObj.pid" placeholder="请选择项目">
                        <el-option v-for="item in (pidList.filter(e=>e!==pid))" :key="item.pid" :label="item.name" :value="item.pid"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item  label="上线时间">
...
<script>
import { blukUpdateVideos, cloneContent, downVideo, getManyVideos, getPublisher, updateCoverURL, updateState, updateVideos, videosExcel } from '@/api/videoManager';
...
export default {
    ...
    methods: {
        ...
        async loadData2() {
            let res = await this.$http(getManyVideos, { type: "video", page: this.page, count: this.count, ...this.sortT });
            if (res.code === 200) {
        ...
        async loadData() {
            let res = await this.$http(getManyVideos, { type: "video", page: this.page, count: this.count, ...this.getQuery() });
            if (res.code === 200) {
                this.isCheckedTags = false;
