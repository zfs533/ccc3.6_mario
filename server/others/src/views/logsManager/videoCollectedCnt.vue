<template>
    <el-card>
        ...
        <el-table :data="pageData" :border="true" min-width="100%">
            ...
            <el-table-column prop="cnt" label="收藏数量" align="center"></el-table-column>
            <el-table-column prop="channel" label="渠道" align="center" :formatter="$channelFormat">
            </el-table-column>
...
<script>
import { getVideoCollectedCnt } from '@/api/logsManager';
import { platFormType, pidList } from '@/utils/baseConst';
export default {
    data() {
        return {
            page: 1,
    ...
    methods: {
        ...
        async loadData() {
            let res = await this.$http(getVideoCollectedCnt, { page: this.page, count: this.count, ...this.getQuery() });
            if (res.code === 200) {
