<template>
    <el-card>
        ...
        <el-table :data="pageData" :border="true" min-width="100%">
            ...
            <el-table-column prop="loginMethod" label="登录方式" align="center" :formatter="loginMethodFormat"></el-table-column>
...
<script>
import { userLoginLog } from '@/api/logsManager';
import { loginMethods, platFormType, pidList } from '@/utils/baseConst';
export default {
    data() {
        return {
            page: 1,
    ...
    methods: {
        ...
        async loadData() {
            let res = await this.$http(userLoginLog, { page: this.page, count: this.count, ...this.getQuery() });
            if (res.code === 200) {
        ...
        loginMethodFormat(row, column, cellValue) {
            let item = loginMethods.find(i => i.value === cellValue);
