System.register(["cc"], function (_export, _context) {
  ...
  return {
    ...
    execute: function () {
      ...
      __checkObsolete__(['_decorator', 'Prefab', 'Node', 'SpriteComponent', 'SpriteFrame', 'Rect', 'ImageAsset', 'resources', 'error', 'Texture2D', 'instantiate', 'isValid', 'find', 'loader', 'Color', 'Primitive', 'Sprite']);
      ...
      _export("resourceUtil", resourceUtil = (_dec = ccclass("resourceUtil"), _dec(_class = class resourceUtil {
        ...
        static loadEnemyPrefabRes(modulePath) {
          return new Promise((resolve, reject) => {
            this.loadRes("prefab/enemy/" + modulePath, Prefab, (err, prefab) => {
              if (err) {
                console.error('effect load failed', modulePath);
                reject && reject();
                return;
        ...
        static createMap(path, cb, parent) {
          this.getMapPrefabRes(path, function (err, prefab) {
            if (err) return;
            var node = instantiate(prefab); // node.setPosition(0, 0, 0);
        ...
        static setSpriteIcon(path, sprite, cb) {
...
//# sourceMappingURL=750d98633ec4b3d1a43f14c7344bdb656871ecf9.js.map